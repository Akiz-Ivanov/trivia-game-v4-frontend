...
const TriviaAddons = ({
  ...
  handleFiftyFifty,
  removedAnswers,
}: TriviaAddonsProps): React.JSX.Element => {
  //*====== State ======
  const [gameAddons, setGameAddons] = useState<GameAddonsState>({
    hint: "",
    info: "",
  });
  ...
  const handleAddonClick = () => {
    if (animations && buttonRef.current) {
      setIsAnimating(true);
    ...
    aiApi[method]({ question: qText, answer: correct_answer, category, signal })
      ...
      .catch((error) => {
        ...
        setGameAddons((prev) => ({
          ...prev,
          [stateObjectKey]: "Sorry, couldn't fetch hint.",
        }));
      })
      .finally(() => {
        setIsLoading(false);
    ...
    <div className="game-addons-container flex flex-col items-center justify-center gap-3-16 py-2 w-full xs:w-auto">
      <div className="flex items-center justify-center gap-8 w-fit bg-[rgba(255,255,255,0.05)] rounded-2xl p-2">
        <Button
          ...
          aria-label={
            selectedAnswer
              ? "Get trivia info from the AI"
              : "Get hint from the AI"
          }
          className={`main-action purple-bg  ${isAnimating ? "animating" : ""}
