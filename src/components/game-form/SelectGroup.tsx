import { FORM_OPTIONS } from "@/data/formConfig";
...
const SelectGroup = ({
  ...
  formData,
  isHalloween,
}: SelectType): React.JSX.Element => {
  //* ====== Create select elements for each key in data object ======
  const selects = Object.entries(FORM_OPTIONS).map(([key, value]) => {
    ...
    return (
      <div key={key} className="select-wrapper w-full space-y-4">
        <label
          htmlFor={key}
          className={cn("w-full", isHalloween && "text-card-foreground")}
        >
          {labelText}
        </label>
        <Select
        ...
        >
          <SelectTrigger
            id={key}
            className={cn(
              "main-action w-full mt-2.5 cursor-pointer px-4",
  ...
  return (
    <div className="flex flex-col gap-4 text-shadow-[1px_1px_2px_#000]">
      {selects}
    </div>
  );
};
